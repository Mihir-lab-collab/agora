...
<script language="VB" runat="server">
    ...
    Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
        ...
        If Month(Now().AddDays(-20)) > 3 Then
            currStartDate = "1-Apr-" & Year(Now().AddDays(-20))
            currEndDate = "31-Mar-" & Year(Now().AddDays(-20)) + 1
           <%-- 
            Prorata = (Month(Now().AddDays(-20)) - 4) / 12
             If (Prorata.ToString() = "0") Then
                    Prorata = 1
                End If--%>
        Else
            currStartDate = "1-Apr-" & Year(Now().AddDays(-20)) - 1
            currEndDate = "31-Mar-" & Year(Now().AddDays(-20))
            Prorata = (Month(Now().AddDays(-20)) + 8) / 12
        End If
        conn1.Open()
    ...
    Function test(ByVal empId As String, ByVal empJoiningDate As String, ByVal empConfDate As String) As String
     ...
     if ((CDate(empConfDate)>CDate(currStartDate)) and (CDate(empConfDate)<CDate(currEndDate))) Then
      ...
      "' AND '" & currEndDate & "' GROUP BY attStatus"
        ...
        If DateAdd("m", 12, empJoiningDate) > Now().AddDays(-20) Then
            empTotPL = 0
            empTotPLC = 0
        End If

        If empConfDate > Now().AddDays(-20) Then
            empTotCL = 0
            empTotCLC = 0
        End If
        If empTotPL > 90 Then
...
function call_Ajax(id)
{
	...
	window.open(url,'pop','width=400;height=300');
