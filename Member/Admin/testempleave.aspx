    ...
    Function test(ByVal empId As String, ByVal empJoiningDate As String, ByVal empConfDate As String) As String
     ...
     if ((CDate(empConfDate)>CDate(currStartDate)) and (CDate(empConfDate)<CDate(currEndDate))) Then
      ...
      "' AND '" & currEndDate & "' GROUP BY attStatus"
        ...
        sql = "select empID,count(coID)as compOff from empCompOff  where empID=" & empId &" and  month(codate)="& DateTime.Now.Month.ToString()& " and year(codate)=" & DateTime.Now.Year.ToString() & " group by empID"
        objcmd = New SqlCommand(sql, conn)
        objdatareader = objcmd.ExecuteReader
        Do While objdatareader.Read()
		...
		sql ="SELECT count(*) as 'TotalCO'  FROM empAtt WHERE upper(attStatus)='CO' and empId=" & empId &" and  month(attDate)="& DateTime.Now.Month.ToString()& " and year(attDate)=" & DateTime.Now.Year.ToString()
        objcmd = New SqlCommand(sql, conn)
        objdatareader = objcmd.ExecuteReader
        Do While objdatareader.Read()
